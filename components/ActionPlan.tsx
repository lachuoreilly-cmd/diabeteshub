...
import { 
  ...
  Sunrise, Sun, Moon as MoonIcon, TrendingUp, History as HistoryIcon
} from 'lucide-react';
import { User, Profile, ExercisePlan, RecipeRecommendation, ExerciseSession } from '../types';
...
const ActionPlan: React.FC<ActionPlanProps> = ({ user, activeProfile, onUpdateProfile }) => {
  ...
  const toggleSaveRecipe = (recipe: RecipeRecommendation) => {
    ...
    if (isSaved) onUpdateProfile({ savedRecipes: activeProfile.savedRecipes.filter(r => r.name !== recipe.name) });
    else onUpdateProfile({ savedRecipes: [...(activeProfile.savedRecipes || []), recipe] });
  };
      ...
      <div className="min-h-[700px] relative">
            ...
            {isWorkoutActive && currentWorkout ? (
              <div className="bg-white rounded-[4rem] p-10 md:p-16 text-slate-900 shadow-2xl border border-blue-100 relative overflow-hidden flex flex-col min-h-[800px]">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center relative z-20 mb-16 gap-8">
                  <div className="flex items-center space-x-6">
                    <div className="p-4 bg-blue-600 rounded-3xl shadow-2xl shadow-blue-500/20"><Timer className="w-8 h-8 text-white" /></div>
                    <div>
                      <h3 className="text-3xl font-black tracking-tight text-slate-900">{currentWorkout.plan.name}</h3>
                      <p className="text-blue-600 font-black text-xs uppercase tracking-widest mt-1">Exercise {currentWorkout.stepIndex + 1} of {currentWorkout.plan.exercises.length}</p>
                    </div>
                  </div>
                ...
                {currentWorkout.status !== 'complete' ? (
                  ...
                  <div className="flex-grow flex flex-col items-center justify-center text-center space-y-12 py-20 animate-in zoom-in-95 duration-700">
                    ...
                    <button onClick={handleFinalizeSession} className="px-16 py-6 bg-blue-600 text-white font-black rounded-[2.5rem] hover:bg-blue-700 transition-all shadow-2xl active:scale-95 text-lg">Commit to Backend</button>
                  </div>
                )}
              </div>
            ) : (
              <div className="text-center space-y-16 bg-blue-50/20 rounded-[4rem] p-12 md:p-32 border border-blue-50 shadow-sm relative overflow-hidden">
                ...
                <div>
                   ...
                   <p className="text-slate-500 mt-4 font-medium max-w-lg mx-auto text-lg leading-relaxed">Select a training routine to begin a real-time health outcome simulation.</p>
                </div>
                <div className="grid gap-6 max-w-2xl mx-auto">
                   {activeProfile.myExercisePlans?.map(plan => (
                     <button key={plan.id} onClick={() => handleStartWorkout(plan)} className="group p-10 bg-white rounded-[3rem] border border-blue-100 flex items-center justify-between hover:border-blue-500 transition-all hover:shadow-2xl">
                ...
                {exerciseDatabase.map(plan => {
                  ...
                  return (
                    <div key={plan.id} className="bg-white p-10 rounded-[4rem] border border-blue-50 shadow-sm flex flex-col group hover:border-blue-400 transition-all hover:shadow-2xl hover:-translate-y-2">
                       ...
                       <button onClick={() => handleAddPlan(plan)} disabled={isAdded} className={`w-full py-6 rounded-[2.25rem] font-black uppercase tracking-widest text-xs transition-all ${isAdded ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                         {isAdded ? 'Module Scheduled' : 'Add to Week'}
                       </button>
                    </div>
                  );
                })}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-12 animate-in fade-in duration-700">
             ...
             {activeProfile.exerciseSessions && activeProfile.exerciseSessions.length > 0 && (
               <section className="bg-slate-900 text-white p-12 rounded-[4rem] shadow-xl">
                 <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center space-x-3">
                       {/* Fix: Using HistoryIcon to avoid conflict with global History object */}
                       <HistoryIcon className="w-6 h-6 text-blue-400" />
                       <h3 className="text-2xl font-black">Session Ledger</h3>
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Persistent Records</span>
                 </div>
                 <div className="space-y-4">
                    {activeProfile.exerciseSessions.slice(0, 5).map(session => (
                      <div key={session.id} className="flex items-center justify-between p-6 bg-white/5 border border-white/10 rounded-[2rem]">
                        <div>
                          <p className="font-bold text-lg">{session.planName}</p>
                          <p className="text-xs text-slate-500">{new Date(session.timestamp).toLocaleDateString()} • {session.duration} mins</p>
                        </div>
                        <div className="bg-emerald-500/20 text-emerald-400 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">Complete</div>
                      </div>
                    ))}
                 </div>
               </section>
        ...
        {activeTab === 'meals' && (
          <div className="space-y-16 animate-in fade-in duration-700">
                   ...
                   <div className="space-y-8">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['Vegetarian', 'Vegan', 'Non-Veg', 'Keto', 'Paleo'].map(p => (
                          <button key={p} onClick={() => setLocalDietPreference(p)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${localDietPreference === p ? 'bg-white text-blue-600' : 'bg-white/10 text-white hover:bg-white/20'}`}>{p}</button>
                        ))}
                      </div>
             ...
             <div className="grid md:grid-cols-3 gap-8">
                {Object.values(groupedMeals).flat().map(meal => (
                  <div key={meal.name} className="bg-white p-10 rounded-[3rem] border border-blue-50 shadow-sm flex flex-col group hover:border-blue-400 transition-all hover:shadow-2xl">
                     <div className="flex justify-between items-start mb-6">
                        <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">{meal.session}</span>
                        <button onClick={() => toggleSaveRecipe(meal)} className={isRecipeSaved(meal.name) ? 'text-emerald-500' : 'text-slate-300'}><BookmarkCheck className="w-5 h-5" /></button>
                     </div>
                     <h4 className="text-2xl font-black text-slate-900 leading-tight mb-8 flex-grow">{meal.name}</h4>
                     <button onClick={() => setSelectedRecipe(meal)} className="w-full py-5 bg-blue-600 text-white font-black rounded-[2rem] hover:bg-blue-700 transition-all">Full Intelligence</button>
                  </div>
                ))}
             </div>
          </div>
