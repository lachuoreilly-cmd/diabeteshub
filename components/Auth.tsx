...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  }, [isLogin]); // Run on mount and toggle between Login/Register
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let message = err.message || "Connection refused. Please try again.";
      try {
        // Try to parse if it's our JSON error format
        const parsed = JSON.parse(message);
        if (parsed.error) {
          if (parsed.error.includes('offline')) {
            message = "Server is currently unreachable. Please check your internet connection or firewall settings.";
          } else {
